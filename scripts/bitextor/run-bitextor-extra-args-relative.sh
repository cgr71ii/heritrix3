...
syntax_error() {
  >&2 echo "Syntax: <experiment_name> <experiment_warcs_dir_or_file> <quantity_warcs_to_process> <work_dir> <lang1> <lang2> <translation_direction> <translation_bash_script_with_options> <bicleaner_ai_metadata_file> [<bitextor_force>] <percentage>"
...
if [[ -z "$EXPERIMENT_NAME" ]] || [[ -z "$EXPERIMENT_WARCS_DIR_OR_FILE" ]] || ([[ ! -d "$EXPERIMENT_WARCS_DIR_OR_FILE" ]] && [[ ! -f "$EXPERIMENT_WARCS_DIR_OR_FILE" ]]) || \
   ...
   [[ ! "$TRANSLATION_DIRECTION" =~ ^[a-z][a-z]2[a-z][a-z]$ ]] || [[ ! -f "$BICLEANER_AI_YAML" ]] || [[ -z "$PERCENTAGE" ]]; then
  syntax_error
fi

WORK="${DIR}/${WORK_DIR}/${EXPERIMENT_NAME}/${PERCENTAGE}_percentage_warcs_processed"
